...
import { ChefHat, ShoppingCart, Bookmark, Search, User } from 'lucide-react';

interface HeaderProps {
  cartCount?: number;
}

export const Header: React.FC<HeaderProps> = ({ cartCount = 3 }) => {
  return (
    <header className="glass-panel sticky top-0 z-50 px-6 py-4 mb-6 mx-4 mt-2 flex items-center justify-between shadow-xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <button className="flex items-center gap-2 bg-gradient-to-r from-yellow-400 to-amber-500 text-slate-900 font-semibold px-4 py-2 rounded-full text-sm hover:brightness-110 shadow-lg transition-all">
          <User size={16} />
          <span>카카오 로그인</span>
        </button>
      </div>
